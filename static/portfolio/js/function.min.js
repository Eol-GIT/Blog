...
(TxtType.prototype.tick = function () {
    ...
    this.isDeleting && (n /= 2),
        this.isDeleting || this.txt !== i ? this.isDeleting && "" === this.txt && ((this.isDeleting = !1), this.loopNum++, (n = 500)) : ((n = this.period), (this.isDeleting = !0)),
        setTimeout(function () {
            e.tick();
        }, n);
}),
(window.onload = function () {
    ...
    (s.type = "text/css"), (s.innerHTML = ".typewrite > .wrap { border-right: 0.1em solid #ffec17; animation: caret 0.75s steps(1) infinite;}"), document.body.appendChild(s);
});
var $skills = $(".skills li"),
    $bars = $(".iconic-bar-chart-bar");
function animateBar() {
    var t = $(this).attr("data-bar");
    $bars.filter(".iconic-bar-chart-bar-" + t).css("transform", "scaleY(1)");
}
function removeBar() {
    var t = $(this).attr("data-bar");
    $bars.filter(".iconic-bar-chart-bar-" + t).css("transform", "scaleY(0)");
}
$skills.hover(animateBar, removeBar), $("section").fadeInScroll({ minDistance: (60 * $(window).height()) / 100 });
